...
class DessertCateg extends StatefulWidget {
  const DessertCateg({super.key, required this.priColor, required this.title});
  final Color priColor;
  final String title;

  @override
  State<DessertCateg> createState() => _DessertCategState();
}

class _DessertCategState extends State<DessertCateg>
  ...
  void initState() {
    ...
    filteredSoupList = List.from(hausaDessertList);
  ...
  void _filterSoupList() {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        filteredSoupList = hausaDessertList.where((soup) {

...
class YorubaSoupCateg extends StatefulWidget {
  const YorubaSoupCateg(
      {super.key, required this.priColor, required this.title});
  final Color priColor;
  final String title;

  @override
  State<YorubaSoupCateg> createState() => _SoupCategState();
}

class _SoupCategState extends State<YorubaSoupCateg>
  ...
  void initState() {
    ...
    filteredSoupList = List.from(yorubaSoupList);
  ...
  void _filterSoupList() {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        filteredSoupList = yorubaSoupList.where((soup) {
